...
class AdminSystemConfigChangedPaymentObserver implements ObserverInterface 
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        foreach($commonConfigKeys as $configKey) {
            $configVal = $this->_appConfig->getConfigDataValue('payment/ambient_payfort/'. $configKey);
    ...
    protected function savaCommonConfig($configKey, $configValue, $scope, $scopeId) {
        $this->_resourceConfig->saveConfig('payment/ambient_payfort_cc/'.$configKey, $configValue, $scope, $scopeId);
        $this->_resourceConfig->saveConfig('payment/ambient_payfort_installments/'.$configKey, $configValue, $scope, $scopeId);
        $this->_resourceConfig->saveConfig('payment/ambient_payfort_naps/'.$configKey, $configValue, $scope, $scopeId);
        $this->_resourceConfig->saveConfig('payment/ambient_payfort_sadad/'.$configKey, $configValue, $scope, $scopeId);
    }
}

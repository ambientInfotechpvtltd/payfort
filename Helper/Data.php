<?php
namespace Ambient\Payfort\Helper;
...
class Data extends \Magento\Payment\Helper\Data
{
    ...
    const AMBIENT_PAYFORT_INTEGRATION_TYPE_REDIRECTION = 'redirection';
    const AMBIENT_PAYFORT_INTEGRATION_TYPE_MERCAHNT_PAGE = 'merchantPage';
    const AMBIENT_PAYFORT_INTEGRATION_TYPE_MERCAHNT_PAGE2 = 'merchantPage2';
    const AMBIENT_PAYFORT_PAYMENT_METHOD_CC = 'ambient_payfort_cc';
    const AMBIENT_PAYFORT_PAYMENT_METHOD_INSTALLMENTS = 'ambient_payfort_installments';
    const AMBIENT_PAYFORT_PAYMENT_METHOD_NAPS = 'ambient_payfort_naps';
    const AMBIENT_PAYFORT_PAYMENT_METHOD_SADAD = 'ambient_payfort_sadad';
    ...
    {
        return $this->scopeConfig->getValue(
            ('payment/ambient_payfort/'.$config_field),
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    }

    public function isAmbientPaymentMethod($paymentMethod) {
        if (preg_match('#^ambient\_payfort\_#', $paymentMethod)) {
            return true;
        }
        return false;
    }

    public function getPaymentRequestParams($order, $integrationType = self::AMBIENT_PAYFORT_INTEGRATION_TYPE_REDIRECTION)
    {
        ...
        if ($integrationType == self::AMBIENT_PAYFORT_INTEGRATION_TYPE_REDIRECTION) {
            $baseCurrency                    = $this->getBaseCurrency();
            $orderCurrency                   = $order->getOrderCurrency()->getCurrencyCode();
            $currency                        = $this->getPayfortCurrency($baseCurrency, $orderCurrency);
            $amount                          = $this->convertPayfortAmount($order, $currency);
            ...
            $gatewayParams['return_url']     = $this->getReturnUrl('ambientpayfort/payment/responseOnline');
            if ($paymentMethod == self::AMBIENT_PAYFORT_PAYMENT_METHOD_SADAD) {
                $gatewayParams['payment_option'] = 'SADAD';
            }
            elseif ($paymentMethod == self::AMBIENT_PAYFORT_PAYMENT_METHOD_NAPS) {
                $gatewayParams['payment_option']    = 'NAPS';
                $gatewayParams['order_description'] = $orderId;
            }
            elseif ($paymentMethod == self::AMBIENT_PAYFORT_PAYMENT_METHOD_INSTALLMENTS) {
                $gatewayParams['installments'] = 'STANDALONE';
                $gatewayParams['command']      = 'PURCHASE';
            }
        }
        elseif ($integrationType == self::AMBIENT_PAYFORT_INTEGRATION_TYPE_MERCAHNT_PAGE || $integrationType == self::AMBIENT_PAYFORT_INTEGRATION_TYPE_MERCAHNT_PAGE2) {
            $gatewayParams['service_command'] = 'TOKENIZATION';
            $gatewayParams['return_url']      = $this->getReturnUrl('ambientpayfort/payment/merchantPageResponse');
            if($paymentMethod == self::AMBIENT_PAYFORT_PAYMENT_METHOD_INSTALLMENTS && $integrationType == self::AMBIENT_PAYFORT_INTEGRATION_TYPE_MERCAHNT_PAGE){
                $baseCurrency                           = $this->getBaseCurrency();
                $orderCurrency                          = $order->getOrderCurrency()->getCurrencyCode();
                $currency                               = $this->getPayfortCurrency($baseCurrency, $orderCurrency);
                $gatewayParams['currency']              = strtoupper($currency);
                $gatewayParams['installments']          = 'STANDALONE';
                $gatewayParams['amount']                = $this->convertPayfortAmount($order, $currency);
            }
        }
        ...
        $this->log("Ambient Request Params for payment method ($paymentMethod) \n\n" . print_r($gatewayParams, 1));
    ...
    public function getPaymentPageRedirectData($order) {
        ...
        return $this->getPaymentRequestParams($order, self::AMBIENT_PAYFORT_INTEGRATION_TYPE_REDIRECTION);
    }
    ...
    public function isMerchantPageMethod($order) {
        $paymentMethod = $order->getPayment()->getMethod();
        if($paymentMethod == \Ambient\Payfort\Model\Method\Cc::CODE && $this->getConfig('payment/ambient_payfort_cc/integration_type') == \Ambient\Payfort\Model\Config\Source\Integrationtypeoptions::MERCHANT_PAGE) {
            return true;
        }
        elseif($paymentMethod == \Ambient\Payfort\Model\Method\Installments::CODE && $this->getConfig('payment/ambient_payfort_installments/integration_type') == \Ambient\Payfort\Model\Config\Source\Integrationtypeoptions::MERCHANT_PAGE) {
            return true;
        }
        return false;
    }
    ...
    public function isMerchantPageMethod2($order) {
        $paymentMethod = $order->getPayment()->getMethod();
        if($paymentMethod == \Ambient\Payfort\Model\Method\Cc::CODE && $this->getConfig('payment/ambient_payfort_cc/integration_type') == \Ambient\Payfort\Model\Config\Source\Integrationtypeoptions::MERCHANT_PAGE2) {
            return true;
        }
        return false;
    }

    public function merchantPageNotifyPayfort($payfortParams, $order) {
        //send host to host
        $orderId = $order->getRealOrderId();

        $return_url = $this->getReturnUrl('ambientpayfort/payment/responseOnline');
        ...
        $postData = array(
            ...
            'token_name'            => $payfortParams['token_name'],
            'language'              => $language,
        ...
        if($paymentMethod == self::AMBIENT_PAYFORT_PAYMENT_METHOD_INSTALLMENTS) {
            $postData['installments'] = 'YES';
            $postData['plan_code']    = $payfortParams['plan_code'];
            $postData['issuer_code']  = $payfortParams['issuer_code'];
        ...
        $debugMsg = "Payfort Merchant Page Notifiaction Response Parameters for payment method ($paymentMethod)"."\n".print_r($response, true);
        $this->log($debugMsg);
     ...
     * @return string payfort signature
     */
     ...
     * @return decimal
     */
    public function convertPayfortAmount($order, $currencyCode)
    {
    ...
    public function getPayfortCurrency($baseCurrencyCode, $currentCurrencyCode)
    {
    ...
    public function getLanguage() {
        ...
        if ($language == \Ambient\Payfort\Model\Config\Source\Languageoptions::STORE) {
    ...
    public function cancelCurrentOrder($comment)
    {
        ...
        if(!empty($comment)) {
            $comment = 'Ambient_Payfort :: ' . $comment;
        }
    ...
    public function processOrder($order, $responseParams, $responseMode = 'online') {
        ...
        if ($responseMode == 'offline' && $order->getState() != $order::STATE_PROCESSING) {
            ...
            $payment->setTransactionId($responseParams['payfort_id'])->setIsTransactionClosed(1);
            $payment->save();

            if($this->getMainConfigData('command') == \Ambient\Payfort\Model\Config\Source\Commandoptions::AUTHORIZATION) {
            ...
            $order->addStatusToHistory( $order::STATE_PROCESSING , 'Ambient_Payfort :: Order has been paid.', true );
            $order->save();
    ...
    private function createInvoice(&$order, $responseParams)
    {
            if (!$order->hasInvoices()) {
                    ...
                    $invoice->register();
                    $invoice->setTransactionId($responseParams['payfort_id']);
     ...
     * @retrun boolean
     */
    public function handlePayfortResponse($payfortParams = array(), $responseMode = 'online', $integrationType = self::AMBIENT_PAYFORT_INTEGRATION_TYPE_REDIRECTION, $responseSource = '')
    {
        try {
            $responseParams  = $payfortParams;
            $success         = false;
            $responseMessage = __('You have canceled the payment, please try again.');

            if (empty($responseParams)) {

                $this->log('Invalid payfort response parameters (' . $responseMode . ')');
                throw new \Exception($responseMessage);
            }

            if (!isset($responseParams['merchant_reference']) || empty($responseParams['merchant_reference'])) {
                $this->log("Invalid payfort response parameters. merchant_reference not found ($responseMode) \n\n" . print_r($responseParams, 1));
                throw new \Exception($responseMessage);
            }
            ...
            $paymentMethod = $order->getPayment()->getMethod();
            $this->log("Payfort response parameters ($responseMode) for payment method ($paymentMethod) \n\n" . print_r($responseParams, 1));

            $notIncludedParams = array('signature', 'ambient_payfort', 'integration_type');
            ...
            if ($responseSource == 'h2h') {
                if ($responseCode == \Ambient\Payfort\Model\Payment::PAYMENT_STATUS_3DS_CHECK && isset($responseParams['3ds_url'])) {
                    if($integrationType == self::AMBIENT_PAYFORT_INTEGRATION_TYPE_MERCAHNT_PAGE) {
            ...
            if (substr($responseCode, 2) != '000') {
                if ($responseCode == \Ambient\Payfort\Model\Payment::PAYMENT_STATUS_CANCELED) {
                    $responseMessage = __('You have canceled the payment, please try again.');
                    if ($responseMode == 'offline') {
            ...
            if (substr($responseCode, 2) == '000') {
                if ($responseMode == 'online' && $responseSource != 'h2h' && (($paymentMethod == self::AMBIENT_PAYFORT_PAYMENT_METHOD_CC || $paymentMethod == self::AMBIENT_PAYFORT_PAYMENT_METHOD_INSTALLMENTS) && ($integrationType == self::AMBIENT_PAYFORT_INTEGRATION_TYPE_MERCAHNT_PAGE || $integrationType == self::AMBIENT_PAYFORT_INTEGRATION_TYPE_MERCAHNT_PAGE2))) {
                    $host2HostParams = $this->merchantPageNotifyPayfort($responseParams, $order);
                    return $this->handlePayfortResponse($host2HostParams, 'online', $integrationType, 'h2h');
                }
                else { //success order
    ...
    public function log($messages, $forceLog = false) {
        ...
        $debugMsg = "=============== Ambient_Payfort Module =============== \n".$messages."\n";
